...
void main() {
  ...
  float coordcenterdist = sqrt((vertTexCoord.x-0.5)*(vertTexCoord.x-0.5) +(vertTexCoord.y-0.5)*(vertTexCoord.y-0.5));
  offset = (sin(coordcenterdist*50)+1)/2;
  vec4 shiftvec = 50*offset * vec4(normal.xyz,0.0);
  // offset = vertTexCoord.x;
  gl_Position = transform * (vertex + shiftvec);
}
