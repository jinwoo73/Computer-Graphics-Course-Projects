...
class Triangle:
    def __init__(self,v1,v2,v3,material):
        ...
        ab = PVector.sub(v2,v1)
        bc = PVector.sub(v3,v2)
        before = PVector.cross(ab,bc)
        self.surfacenormal = before.normalize()
    
    def intersect(self,ray):
        denom = self.surfacenormal.dot(ray.direction)
    
        uppernom = self.surfacenormal.dot(PVector.sub(self.v1,ray.origin))
        if denom == 0:
            return 0
        t = uppernom/denom
        return t
